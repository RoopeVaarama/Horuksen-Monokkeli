...
export class SearchService {
  ...
  findValue(terms: Terms, page: PDFExtractPage, key_x: number, key_y: number) {
    ...
    if (
      (terms.direction == Direction.Right ||
        terms.direction == Direction.Left) &&
      candidates.length > 0
    )
      return this.closestOnX(key_x, candidates);
    if (
      (terms.direction == Direction.Above ||
        terms.direction == Direction.Below) &&
      candidates.length > 0
    )
  ...
  closestOnX(x: number, values: PDFExtractText[]) {
    ...
    for (i = 1; i < values.length; ++i) {
      if (Math.abs(x - values[i].x) < Math.abs(x - closest.x))
        closest = values[i];
    }
    return closest;
  }
  ...
  closestOnY(y: number, values: PDFExtractText[]) {
    ...
    let closest: PDFExtractText = values[0];
    for (i = 1; i < values.length; ++i) {
      if (Math.abs(y - values[i].y) < Math.abs(y - closest.y))
        closest = values[i];
    }
    return closest;
  }
