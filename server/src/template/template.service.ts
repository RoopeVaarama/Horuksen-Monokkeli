...
import {
  ValueSearch,
  ValueSearchDocument,
} from './schemas/value-search.schema';

@Injectable()
export class TemplateService {
  constructor(
    @InjectModel(ValueSearch.name)
    private templateModel: Model<ValueSearchDocument>,
  ) {}
  ...
  async getTemplates(uid: string): Promise<ValueSearch[]> {
    ...
    if (!templates.length)
      throw new NotFoundException('No templates found with that user id');
    return templates;
  }
  ...
  async getTemplate(tid: string): Promise<ValueSearch> {
    if (!Types.ObjectId.isValid(tid))
      throw new NotAcceptableException('Not a valid template id');
    const template = await this.templateModel.findOne({ _id: tid }).exec();
    if (!template)
      throw new NotFoundException('No template found with that template id');
    return template;
  }
}
