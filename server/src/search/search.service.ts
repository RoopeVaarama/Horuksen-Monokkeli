...
export class SearchService {
	...
	private findValue(search: Search, page: PDFExtractPage, key_x: number, key_y: number) {
		const contentArray = page.content;
		let entryIndex: number;
	...
	private inMargin(val: number, key: number) {
		return val >= key - margin && val <= key + margin;
}

	private closestOnX(x: number, values: PDFExtractText[]){
		let i: number;
	...
	private closestOnY(y: number, values: PDFExtractText[]) {
		let i: number;
		let closest: PDFExtractText = values[0];
		for (i = 1; i < values.length; ++i) {
	...
	private keyMatch(key: string, entry: string) {
		return key == entry;
	}

	// TODO value matching, regex?
	private valueMatch(value: string, regex: string) {
		return true;
    }
